...
const registerIntoDB = async (payload, fileBuffer) => {
...
const verifyEmail = async (payload) => {
    ...
    const accessToken = jwtUtiles.createToken(jwtPayload, config.jwt_access_secret, config.jwt_access_expiration);
    const refreshToken = jwtUtiles.createToken(jwtPayload, config.jwt_access_secret, config.jwt_access_expiration);
    return {
        accessToken,
        refreshToken,
        createUser,
...
const userloginFromBD = async (payload) => {
    ...
    if (!user.password) {
        throw new Error("This account does not have password login enabled.Try with Google ,Facebook or others");
    }
...
const refreshToken = async (token) => {
    const verifiedRefreshToken = jwtUtiles.varifyToken(token, config.jwt_refresh_secret);
    if (!verifiedRefreshToken.success || !verifiedRefreshToken.data) {
        throw new Error(config.node_env === "development"
    ...
    const accessToken = jwtUtiles.createToken(jwtPayload, config.jwt_access_secret, config.jwt_access_expiration);
    const refreshToken = jwtUtiles.createToken(jwtPayload, config.jwt_refresh_secret, config.jwt_refresh_expiration);
    return {
        accessToken,
        refreshToken,
    };
